...
class RUDP():
    '''
    Create a socket running RUDP at addr:port
    '''
    ...
    def __init__(self, addr='localhost', port=10000, client=False):
        ...
        if not client:
            ...
            print('Server RUDP running on %s:%s' % self.sock.getsockname())
        else:
            print('Client RUDP running')
        ...
        # ATTENTION:
        # Once invoked the RUDP_recv(bufferSize) function, at most bufferSize data is returned
        ...
        # Congession control state, 0: slowStart, 1:congessionAvoid, 2:fastRecovery
        self.congessState = 0
                ...
                if window_index < self.sendWindowSize:
                    if window[window_index]:  # duplicate ACK
                        ...
                        if self.congessState == utils.slowState:  # new ACK for slow state
                            ...
                            self.cwnd = self.cwnd + \
                                int(self.MSS * (self.MSS / self.sendWindowSize))
                            self.dupACK = 0
                        ...
                        if (ack_index > lastAck and ack_index < lastSend):
                            ...
                            if self.sendWindowSize == 0:  # Start waiting rwnd
                                print(
                                    'SEND: Waiting for receiver to have buffer(rwnd)')
                                rwnd_seq = 0
                                while True:
                                    ...
                                    try:
                                        rwnd_data, rwnd_addr = self.sock.recvfrom(
                                            1024)
                                    except:
                                        continue

                                    rwnd_decode_data = rwnd_data.decode()
                                    rwnd_AckNum = rwnd_decode_data.split('$')[
                                        1]
            ...
            if (self.cnt < num):
                ...
                if (int(decode_ack) and rcv_addr in self.seq and
                    ...
                    self.clientSock.append(
                        [rcv_addr, RUDP(addr=self.getLocalAddr()[0], port=(self.new_port[rcv_addr]))])
            else:
                # break
                print(
    ...
    def accept(self):
        '''
        Retrieve a client-connected RUDP;
    ...
    def release(self):
        '''
        Cancel a client-connected RUDP;
    ...
    def getLocalAddr(self):
        '''
        Return the RUDP running local address in a pair (addr, port)
        '''
