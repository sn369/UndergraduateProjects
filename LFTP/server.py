    ...
    while True:
        ...
        if command == "exit":
            ...
            while threading.active_count() != 1:
                print("Waiting for %d threads to ends." %
...
def listen(hostname, port):
    ...
    server = RUDP.RUDP(addr=hostname, port=int(port))
    listenThread = threading.Thread(
        target=server.listen, args=(10,), name="basic listening")
    listenThread.start()
    while True:
        time.sleep(0.1)
        ...
        if clientSocket != None:
            ...
            handleThread = threading.Thread(
                target=handleSocket, args=(clientSocket,))
            handleThread.start()
        serverLock.release()
...
def handleSocket(socket):
    commandPacket = socket.RUDP_recv(1024)
    ...
    with open("data/" + filename, "wb") as f:
        ...
        while True:
            ...
            if acLength == length:
                ...
                break
            # Receive some data
            metadata = socket.RUDP_recv(60000)
            data = base64.b64decode(metadata.encode("ASCII"))
            if len(data) == 0:
                print(
                    "Receiving %s: Connection Error: Timeout when receiving data." % filename)
                break
            acLength += len(data)
            print("Recv:  %d bytes" % acLength)
            print("Total: %d byets" % length)
            print("Receiving %s: %d%% data received..." %
                  (filename, acLength / length * 100))
            print("Speed: %d KB/s" %
                  (acLength / (time.time() - start_time + 0.01) / 1000))
            # Write to file
            f.write(data)
        ...
        while sentLength != length:
            ...
            for x in range(0, SECTION_NUM):
                line += base64.b64encode(
                    metaLine[x * DIV_SIZE:x * DIV_SIZE + DIV_SIZE]).decode("ASCII")
            print("Sending Length: ", len(line))
            if not socket.RUDP_send(line):
                print("Error while sending file %s." % filename)
                return
            sentLength += len(metaLine)
            print("Sent:  %d bytes" % sentLength)
            print("Total: %d bytes" % length)
            print("Sending file %s: %d%% done." %
                  (filename, sentLength / length * 100))
            print("Speed: %d KB/s" %
                  (sentLength / (time.time() - start_time + 0.01) / 1000))
        print("Sending done.")
...
if len(sys.argv) == 2 and sys.argv[1] == "help":
    ...
    listenThread = threading.Thread(
        target=listen, args=(hostname, port), name="Listen Thread")
    listenThread.start()
    console()
elif len(sys.argv) == 1:
    hostname = "localhost"
