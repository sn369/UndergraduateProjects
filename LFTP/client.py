...
def lSend():
    ...
    with open(sys.argv[3], "rb") as f:
        ...
        if not client.RUDP_send("lsend\n" + filename + "\n" + str(length)):
            print("Error while sending command.")
            return

        print("waiting for response")
        response = client.RUDP_recv(1024)
        if response != "OK":
    ...
    with open(sys.argv[3], "wb") as f:
        ...
        while True:
            if acLength == length:
                ...
                break
            # Receive some data
            metadata = client.RUDP_recv(60000)
